...
lint: ## Check code base according to Plone standards
	...
	@uvx ruff@latest check --config $(BACKEND_FOLDER)/pyproject.toml
	@uvx zpretty@latest --check src

.PHONY: format
format: ## Format code base according to Plone standards
...
i18n: $(VENV_FOLDER) ## Update locales
	...
	@$(BIN_FOLDER)/python -m wcs.keycloak.locales

# Tests
.PHONY: test
test: $(VENV_FOLDER) ## Run tests
	@WSGI_SERVER_PORT=65035 $(BIN_FOLDER)/zope-testrunner --auto-color --auto-progress --test-path=src
