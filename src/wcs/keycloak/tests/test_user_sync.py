...
class TestSyncAllUsers(UserSyncTestBase):
    ...
    def test_sync_creates_users_in_storage(self):
        ...
        self.assertEqual(storage[username]['email'], 'test_usersync_create_001@example.com')
        self.assertEqual(storage[username]['firstName'], 'Sync')
        self.assertEqual(storage[username]['lastName'], 'Test')
        self.assertGreater(stats['users_synced'], 0, 'Should have synced at least one user')
    ...
    def test_sync_updates_existing_user_properties(self):
        ...
        self.client._make_request('PUT', url, json={
        ...
        self.assertEqual(
            storage[username]['email'], 'new_email@example.com',
    ...
    def test_sync_removes_deleted_users_from_storage(self):
        ...
        storage = plugin._get_user_storage()
        self.assertNotIn(
            'nonexistent_user_xyz', storage,
            'User not in Keycloak should be removed from storage',
        )
        self.assertGreater(stats['users_removed'], 0, 'Should have removed at least one user')
