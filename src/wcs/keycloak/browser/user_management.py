...
class KeycloakPluginMixin:
    ...
    def is_controls_enabled(self):
        return api.portal.get_registry_record(
            'wcs.keycloak.show_keycloak_controls', default=False,
        )
    ...
    def _build_sync_message(self, stats):
        messages = [
            f"{stats[key]} {label}"
            for key, label in SYNC_STAT_LABELS
            if stats.get(key)
        ]
        if messages:
...
class KeycloakGroupsOverviewControlPanel(KeycloakPluginMixin, GroupsOverviewControlPanel):
