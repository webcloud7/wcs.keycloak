...
class KeyCloakLayer(BaseDockerServiceLayer):
    ...
    container_name = 'wcs.keycloak_test'
    ...
    def _create_realm(self, filename='test-realm.json'):
        self.admin_session.headers.update({'Content-Type': 'application/json'})

        response = self.admin_session.get(
            'http://localhost:8000/admin/realms/test'
        )
...
class KeycloakPasPluginLayer(PloneSandboxLayer):
...
KEYCLOAKPASPLUGIN_FIXTURE = KeycloakPasPluginLayer()
KEYCLOAKPASPLUGIN_FUNCTIONAL_TESTING = FunctionalTesting(
    bases=(KEYCLOAK_FIXTURE, KEYCLOAKPASPLUGIN_FIXTURE, WSGI_SERVER_FIXTURE, ),
    name='KeycloakPasPlugin:Functional')
